...
const TodoPostForm = (props) => {
  ...
  const reset = async () => {
    await Promise[(setText(null), setPrice(null), setQuant(null))];
  };

  const redirect = async () => {
    await Promise[addTodo(text, quant, price, color)];
    await Promise[reset()];
    await Promise[setColor(arrCo[Math.floor(Math.random() * arrCo.length)])];
    await Promise[props.navigation.navigate('CreateTodo')];
  };

  const {addTodo, doTotal} = useContext(TodoContext);
  return (
    <>
      <View
        style={{
